...
/** Shared challenge-based provider adapter. Aliases are assigned by the installation/slot
 * catalog, never derived from user input or changed after identity preparation. */
export function createProviderKeyPorts(
  options: {
    provider?: "app-attest" | "development";
    issuer: string;
    clientId: string;
  ...
  async function evidence(
    ...
    const value = await call(
      () =>
        native.evidence.generateEvidence(
    ...
    prepare: async (slot, context) => {
      ...
      const key = z
        ...
        .safeParse(
          await call(
            () =>
              native.evidence.getOrCreateKey(
                options.keyIdStoragePrefix,
